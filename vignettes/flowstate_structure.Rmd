...
`[['parameters']]` also contains attribute-level metadata -- notably, an `alias` attribute that serves the purpose of linking syntactically-valid names with their original '$PnN' name.  The aliases are used to rename variables in `'[['data']]`, allowing for non-standard evaluation when programming using `data.table`.

```{r flowstate.parameters.alias}
##
attr(fs$parameters, 'alias')
